...
import {
  ...
  getPageViewTool,
  handleGetPageView,
} from "./get-page-view.js";

describe("Get Page View Tool", () => {
  ...
  describe("ツール定義", () => {
    test("正しいツール名を持つ", () => {
      expect(getPageViewTool.name).toBe("get_confluence_page_view");
    });

    test("説明文が定義されている", () => {
      expect(getPageViewTool.description).toBeDefined();
      expect(getPageViewTool.description).toBeTypeOf("string");
      if (getPageViewTool.description) {
        expect(getPageViewTool.description.length).toBeGreaterThan(0);
      }
    });

    test("入力スキーマが定義されている", () => {
      expect(getPageViewTool.inputSchema).toBeDefined();
      expect(getPageViewTool.inputSchema.type).toBe("object");
      expect(getPageViewTool.inputSchema.properties).toBeDefined();
    });

    test("pageId パラメータが必須として定義されている", () => {
      const props = getPageViewTool.inputSchema.properties;
      if (props && typeof props === "object") {
        expect(props.pageId).toBeDefined();
      }
      expect(getPageViewTool.inputSchema.required).toContain("pageId");
    });
  });

  describe("handleGetPageView", () => {
    describe("バリデーション", () => {
      test("pageIdが空文字列の場合、エラーを返す", async () => {
        ...
        const result = await handleGetPageView(args);
